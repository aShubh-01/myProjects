...
{
	printf("***** Tic-Tac-Toe *****\n\n");  /* Introductory Design */
	char player1[20], player2[20];		/* variables to store player names, select slot number and store marks(x/o) */
	...
	if(markPrint[slotNum-1] != ' ') {
		...
		for(i = 0; i < 3; i++) {	/* or if the selected slot is empty then that slot will be available to mark on */
			...
			for(j = 0; j < 3; j++) {
				...
				printf("%c|", markPrint[count]);	 /* Print all slots i.e print the slot array with all assigned and unassigned marks*/
				count++;
			}
			printf("\n");
		}
...
int checkWin(char markPrint[],char mark) {	
	...
	int i, count = 0;						
	for(i = 0; i < 9; i++) {				/* There are 8 ways to win TTT */ 
		if(markPrint[i] != ' '){			/* i.e 8 combinations of a mark  'x/o' appearing 3 times on board eg. (|x||x||x|) */ 
			count++;						/* if all 9 slots are marked and no combination is seen then */
		}									/* this loops return the value '2' to 'isOver' variable which indicates that */
		if(count == 9) {					/* The game is a tie */
			return 2;
		}
	}
}
